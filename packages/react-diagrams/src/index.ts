...
import { PathFindingLinkFactory, DefaultPathFactory, RightAnglePathFactory } from '@piotrmitrega/react-diagrams-routing';
...
export default (options: CanvasEngineOptions = {}): DiagramEngine => {
	...
	const pathFindingLinkFactory = new PathFindingLinkFactory();
	...
	engine.getLinkFactories().registerFactory(pathFindingLinkFactory);
	engine.getPortFactories().registerFactory(new DefaultPortFactory());

	engine.getPathFactories().registerFactory(new DefaultPathFactory());
	engine.getPathFactories().registerFactory(new RightAnglePathFactory(pathFindingLinkFactory));

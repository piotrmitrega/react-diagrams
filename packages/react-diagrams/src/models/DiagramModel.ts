...
export interface DiagramListener extends BaseEntityListener {
  nodesUpdated?(
    event: BaseEntityEvent & { node: NodeModel; isCreated: boolean },
  ): void;
...
export class DiagramModel<
  G extends DiagramModelGenerics = DiagramModelGenerics
> extends CanvasModel<G> {
  ...
  deserializeLayersModels(
    engine: DiagramEngine,
    model: SerializedDiagramModel,
  ) {
    const models: {
      [id: string]: BaseModel;
    } = {};
  ...
  getLinkLayers(): LinkLayerModel[] {
    return _.filter(
      this.layers,
      (layer) => layer instanceof LinkLayerModel,
    ) as LinkLayerModel[];
  }
  ...
  getNodeLayers(): NodeLayerModel[] {
    return _.filter(
      this.layers,
      (layer) => layer instanceof NodeLayerModel,
    ) as NodeLayerModel[];
  }

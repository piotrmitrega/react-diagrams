...
export class LinkModel<G extends LinkModelGenerics = LinkModelGenerics>
  ...
  getBoundingBox(): Rectangle {
    return Polygon.boundingBoxFromPoints(
      _.map(this.points, (point: PointModel) => point.getPosition()),
    );
  }
  ...
  getSelectionEntities(): Array<BaseModel> {
    if (this.getTargetPort() && this.getSourcePort()) {
      return super
        ...
        .concat(_.slice(this.points, 1, this.points.length - 1));
    }
    // allow selection of the first point
    if (!this.getSourcePort()) {
      return super
        .getSelectionEntities()
        .concat(_.slice(this.points, 0, this.points.length - 1));
    }
    // allow selection of the last point
    if (!this.getTargetPort()) {
      return super
        .getSelectionEntities()
        .concat(_.slice(this.points, 1, this.points.length));
    }
  ...
  serialize(): SerializedLinkModel {
    return {
      ...
      points: _.map(this.points, (point) => point.serialize()),
      labels: _.map(this.labels, (label) => label.serialize()),
    };
  }

  doClone(lookupTable = {}, clone) {
    clone.setPoints(
      _.map(this.getPoints(), (point: PointModel) => point.clone(lookupTable)),
    );
    if (this.sourcePort) {

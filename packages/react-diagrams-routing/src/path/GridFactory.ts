...
> {
  ...
  calculateGridDimensions = (): void => {
    const allNodesCoords = _.values(this.engine.getModel().getNodes()).map(
      (item) => ({
        x: item.getX(),
        width: item.getWidth(),
        y: item.getY(),
    ...
    const sumProps = (object, props) =>
      _.reduce(props, (acc, prop) => acc + _.get(object, prop, 0), 0);

    const canvas = this.engine.getCanvas() as HTMLDivElement;
    const concatedCoords = _.concat(allNodesCoords, allPortsCoords);
    const minX =
      Math.floor(
        Math.min(_.get(_.minBy(concatedCoords, 'x'), 'x', 0), 0) /
          GridFactory.SCALING_FACTOR,
      ) * GridFactory.SCALING_FACTOR;
    const maxXElement = _.maxBy(concatedCoords, (item) =>
    ...
    const minY =
      Math.floor(
        Math.min(_.get(minYCoords, 'y', 0), 0) / GridFactory.SCALING_FACTOR,
      ) * GridFactory.SCALING_FACTOR;
    const maxYElement = _.maxBy(concatedCoords, (item) =>
